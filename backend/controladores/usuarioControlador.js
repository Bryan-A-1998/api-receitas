...
async function cadastrar(req, res) {
  try {
    ...
    const usuarioId = await UsuarioRepositorio.cadastrar(nome, email, senha);

    res.status(201).json({ mensagem: "Usuário cadastrado com sucesso", usuarioId });
  } catch (erro) {
...
async function editar(req, res) {
  try {
    ...
    const atualizado = await UsuarioRepositorio.editar(id, nome, senha);
